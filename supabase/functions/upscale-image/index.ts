...
serve(async (req) => {
  ...
  try {
    ...
    const content = [
      {
        type: "text",
        text: `Upscale this image to ${scaleText} its original resolution. Enhance details, improve sharpness, and maintain high quality. Use advanced super-resolution techniques to add detail while keeping the image natural. Return only the upscaled image.`
      }
    ];
    ...
    console.log(`Processing ${scaleText} upscale...`);
    ...
    if (!resultImage) {
      ...
      return new Response(
        JSON.stringify({ error: "Failed to upscale image. Please try again." }),
        { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    return new Response(
      JSON.stringify({ imageUrl: resultImage, tool: "upscale", scale }),
